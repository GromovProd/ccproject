...
void i2c_t::Detect() {
    ...
    for(uint8_t y=0; y<128; y+=16) {
        ...
        for(uint8_t x=0; x<16; x++) {
            ...
            // Probe address
            uint8_t r = CmdWriteWrite(Addr, NULL, 0, NULL, 0);
            if(Error == true) Reset();
            if(r == OK) Uart.Printf("%02X ", Addr);
            else Uart.Printf("-- ");
        }
    }
}
...
uint8_t i2c_t::CmdWriteRead(uint8_t Addr,
        void *WPtr, uint8_t WLength,
        void *RPtr, uint8_t RLength) {
...
uint8_t i2c_t::CmdWriteWrite(uint8_t Addr,
        void *WPtr1, uint8_t WLength1,
        void *WPtr2, uint8_t WLength2) {
    if(IBusyWait() != OK) return FAILURE;
    // Clear flags
    ii2c->SR1 = 0;
    while(RxIsNotEmpty()) (void)ii2c->DR;   // Read DR until it empty
