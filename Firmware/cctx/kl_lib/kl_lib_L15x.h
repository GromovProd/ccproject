...
public:
    ...
    void Resume();
    void Reset();
    void Detect();  // Scan the bus for devices
    uint8_t CmdWriteRead(uint8_t Addr, void *WPtr, uint8_t WLength, void *RPtr, uint8_t RLength);
    uint8_t CmdWriteWrite(uint8_t Addr, void *WPtr1, uint8_t WLength1, void *WPtr2, uint8_t WLength2);
};
#endif
